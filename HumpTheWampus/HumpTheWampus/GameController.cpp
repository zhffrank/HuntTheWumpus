...
namespace GameController
{
	...
	void HandleMoveCommands(struct GameLocations& info, int& command)
	{
		switch (command)
		{
		case 'j':
	...
	void ToggleCheatMode(struct GameLocations& info) {
		info.ToggleCheatMode();
	}

	void NewGame(struct GameLocations& info) {
		int size;
		do {
	...
	bool IsGameEnded(struct GameLocations& info)
	{
	...
	// k - down
	// l - right
	void HandleCommands(struct GameLocations& info, int& command)
	{
		switch (command)
		{
		case 'j':
